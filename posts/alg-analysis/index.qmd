...
I’ve noticed that a lot of students -- whether they're currently taking algorithms or have already completed the course -- often confuse cases with bounds when discussing algorithm analysis. I know I had to wrestle with this concept myself for quite a while before things started to click. 
...
This is the most pessimistic, but also a reliable way to measure an algorithm’s performance because it guarantees that the algorithm will always run within a certain time, regardless of the input.
...
In algorithm analysis, we often use bounds to describe how an algorithm's running time grows as the input gets larger.
...
In theoretical CS, unless specified otherwise, asymptotic notation usually refers to the **worst-case**. In practice, especially with randomized algorithms or heuristics, **expected or average-case** complexity may be more relevant.
...
This is a tight bound. In other words, the upper and lower bounds are equivalent up to constant factors. Thus, the algorithm is **asymptotically optimal**.

### Little o: $o(f(n))$

Similar to Big O, but shows the algorithm A’s running time is **strictly slower than** $f(n)$. The function $f(n)$ graphed is *strictly above* to algorithm A's graph.

For example, $n$ grows strictly slower than $n \log n$.

### Little omega: $\omega(f(n))$

Similar to Big Omega, but shows the algorithm A’s running time is **strictly faster than** $f(n)$. The function $f(n)$ graphed is *strictly below* to algorithm A's graph.

For example, $n \log n$ grows strictly faster than $n$.

---

In algorithm analysis, we aim to measure performance across different input scenarios using asymptotic notations. Worst-case is most common in theory, but average and expected case are more often used in practice. I hope this clears up some common misconceptions!